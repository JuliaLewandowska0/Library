...
<div class="page">
	...
	<div class="right">
		<form class="searchBar-AllBooksDisplay" action="UserSearchResultL.php" method="POST">
			<input id="searchField-AllBooksDisplay" type="text" name="searchField" placeholder="Wyszukaj tytuł lektury, wydawnictwo lub autora">
			<button id="submitSearchFieldButton-AllBooksDisplay" type="submit" name="submitSearchField">Szukaj</button>
		</form>
		<h2 id="headerDisplayBooks-AllBooksDisplay">Lektury</h2>
		<div id="textUnderDisplayHeader-AllBooksDisplay">Kliknij wybraną lekturę, aby zobaczyć szczegółowe informacje na jej temat.</div>
		<div class="product-wrapper" id="result">
			<?php //Podstawowe wyświetlanie wszystkich lektur, po filtrowaniu lub sortowaniu zmienia się zapytanie SQL
				$pdo = connect();
				$sql="SELECT tytul_ksiazki, imie_nazwisko_autora, nazwa_gatunku, id_ksiazki, okladka FROM ksiazka JOIN autor ON ksiazka.id_autora=autor.id_autora JOIN gatunek ON ksiazka.id_gatunku=gatunek.id_gatunku WHERE czy_lektura=1;";
...
<script src="details_connection.js"></script> <!--Połączenie ze szczegółami lektury ze wzlędu na jej ID-->
...
<script>
	$(document).ready(function() //Funkcja nasłuchująca zmian w filtrach poprzez zaznaczenie i przesyłająca wyniki do pliku modyfikującego zapytanie SQL
	{
		$(".product_check").click(function()
		{
			...
			$.ajax(
			{
				url:'includes/filter_sorting_User_L.php',
				method: 'POST',
				data:{action:action,rok_wydania_ksiazkiDESC:rok_wydania_ksiazkiDESC,nazwa_statusu:nazwa_statusu,
