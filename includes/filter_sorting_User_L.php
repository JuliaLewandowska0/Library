<?php
    ...
    if(isset($_POST['action'])) //Sprawdzenie czy został zaznaczony przycisk sortowania/filtrowania
    {
        ...
        JOIN status ON ksiazka.id_statusu=status.id_statusu
        JOIN gatunek ON ksiazka.id_gatunku=gatunek.id_gatunku  WHERE czy_lektura=1 AND tytul_ksiazki !=''";
        ...
        if(isset($_POST['nazwa_statusu'])) //Dodanie filtrowania po nazwie statusu do zapytania SQL
        {
            $nazwa_statusu = implode("','",$_POST['nazwa_statusu']);
            $sql .= "AND nazwa_statusu IN('".$nazwa_statusu."')";
            $sql_2 .= "AND nazwa_statusu IN('".$nazwa_statusu."')";
        }
        ...
        if($count>0)
        {
            ...
            {
                $output .= '<div class="product" onclick="details_connection_User_L('.$row['id_ksiazki'].')">
                <div class="left2">
